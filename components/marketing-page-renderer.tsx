...
function renderBrandCards(brands: BrandFeature[]) {
  return (
      ...
      {brands.map((brand) => (
        ...
        >
          ...
          <div className="p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-[#4d3b37]">{brand.summary}</p>
            <Link
              href={brand.href}
...
function renderNewsCards(newsItems: NewsItem[]) {
  return (
    <div className="mt-10 grid gap-6 md:grid-cols-3">
      {newsItems.map((story) => (
        <article
          key={story.id}
          className="card-lift overflow-hidden rounded-[1.5rem] border border-black/10 bg-white shadow-[0_14px_30px_rgba(28,20,19,0.08)]"
        >
          <div className="p-5">
            <h3 className="text-4xl font-[family:var(--font-title)] uppercase leading-none text-[#220707]">
              {story.title}
            </h3>
            <p className="mt-4 text-sm leading-7 text-[#4d3b37]">{story.excerpt}</p>
            <Link
              href="/news"
              className="card-lift-reveal mt-4 inline-flex text-sm font-semibold text-[#220707] underline underline-offset-2"
            >
              Read more
            </Link>
          </div>
...
function renderEventCards(events: EventItem[]) {
  return (
    <div className="mt-10 grid gap-6 lg:grid-cols-3">
      {events.map((event) => (
        ...
        >
          ...
          <div className="p-6">
            <p className="text-sm uppercase tracking-[0.12em] text-[#7c5b56]">{event.teaserDate}</p>
            <h3 className="mt-3 text-4xl font-[family:var(--font-title)] uppercase leading-none text-[#220707]">
              {event.title}
            </h3>
            <p className="mt-4 text-sm leading-7 text-[#4d3b37]">{event.summary}</p>
            <Link
              href={`/events/${event.slug}`}
              className="card-lift-reveal mt-5 inline-flex items-center rounded-full bg-[#6b4d48] px-5 py-2.5 text-sm font-semibold text-white"
            >
              Details
            </Link>
          </div>
...
function renderCollectionGrid(block: Extract<PageBlock, { type: "collectionGrid" }>, content: RidemaxSiteContent) {
  if (block.source === "brands") {
    ...
    return renderBrandCards(brands);
  }
  ...
  if (block.source === "news") {
    ...
    return renderNewsCards(newsItems);
  }
  ...
  if (block.source === "events") {
    ...
    return renderEventCards(events);
  }
...
function renderBlock(
  ...
  if (block.type === "brandMarquee") {
    ...
    return (
      <section key={block.id} className="bg-white py-14">
  ...
  if (block.type === "categoryTiles") {
    return (
      <section key={block.id} className="bg-white py-16">
        <div className="mx-auto max-w-[76rem] px-6 md:px-10">
          <SectionHeader eyebrow={block.eyebrow} title={block.title} summary={block.summary} />
  ...
  if (block.type === "imageMarquee") {
    return (
      <section key={block.id} className={`overflow-hidden py-14 ${block.background ?? "bg-[#E31E24] text-white"}`}>
        <div className="mx-auto max-w-[118rem] px-6 md:px-10">
  ...
  if (block.type === "showcase") {
    ...
    if (useBentoLayout) {
      return (
        <section key={block.id} className="relative bg-white pb-16 pt-14">
          <div className="mx-auto max-w-[72rem] px-6 md:px-10">
            <SectionHeader eyebrow={block.eyebrow} title={block.title} summary={block.summary} />
            {block.accent ? (
    ...
    return (
      <section key={block.id} className="relative bg-white pb-16 pt-14">
        <div className="mx-auto grid max-w-[72rem] gap-10 px-6 md:px-10 lg:grid-cols-[1fr_0.9fr]">
  ...
  if (block.type === "jobsList") {
    ...
    return (
      <section key={block.id} className="bg-white py-12">
        <div className="mx-auto max-w-[72rem] px-6 md:px-10">
          <SectionHeader eyebrow={block.eyebrow} title={block.title} summary={block.summary} />
          <CareersJobBrowser
  ...
  if (block.type === "calendar") {
    ...
    return (
      <section key={block.id} className="bg-[#f2efec] py-16">
        <div className="mx-auto max-w-[72rem] px-6 md:px-10">
